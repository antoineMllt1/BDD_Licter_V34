...
export function StrategicHero({ eyebrow, title, summary, whyItMatters, whatNow, actions = [], stats = [] }) {
  const heroPills = [
    whyItMatters ? { label: 'Enjeu', value: compactText(whyItMatters, 8) } : null,
    whatNow ? { label: 'Priorite', value: compactText(whatNow, 9) } : null,
  ].filter(Boolean)
    ...
    <section className="strategic-hero">
      <div className="strategic-hero-copy">
        ...
        <div className="hero-pill-row">
          {heroPills.map((pill) => (
            <div key={pill.label} className="hero-pill">
...
export function SignalCard({ label, value, note, tone = 'neutral' }) {
  return (
        ...
        <div className="evidence-list">
          {items.slice(0, 3).map((item, index) => (
            <div key={item.id || index} className="evidence-item">
              <div className="evidence-quote">"{compactText(item.text, 26)}"</div>
